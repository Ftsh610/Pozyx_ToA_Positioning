...
if __name__ == "__main__":
	...
	# AP 1  : ( 214,    0) -> (   0,    0) 기준으로 전부 좌표계 평행이동하면
	# AP 6  : ( 754, 1589) -> ( 540, 1589)
	# AP 7  : ( -26, 1572) -> (-240, 1572)
	# AP 8  : (   0,  755) -> (-214,  755)
	# AP 12 : ( 714,  755) -> ( 500,  755)

	destination_id = [0x675f, 0x6a5e, 0x672d, 0x6e06, 0x6a32 ]
	...
	#Constants here!!
	#pos0 = []
	H = np.array([[5.4,15.89], [-2.4,15.72], [-2.14,7.55], [5,7.55]])#*1000 # mm unit.
...
while (1):
	...
	ranges_without_0 = np.array([range1.distance,range2.distance,range3.distance,range4.distance])/1000
	...
	# POZYX 모듈과 안테나 사이의 실제거리 보정 (0.07,-0.21) + AP0중심->내가쓰던 좌표계로 이동 (1.2,-2.75)
	position = position + np.array([1.27, -2.96]) 
